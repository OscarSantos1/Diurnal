...
const FloatingBtn = ({ title, onClick, showDone }) => {
  ...
  if (title === "Yesterday") {
    btnIcon = (
      <IoIosRedo size="22" style={{ color: "#4689CC", cursor: "pointer" }} />
    );
    tip = "Add pending tasks to Today";
  } else {
    if (showDone) {
      btnIcon = (
        <IoMdEyeOff size="25" style={{ color: "#4689CC", cursor: "pointer" }} />
      );
      tip = "Hide finished tasks";
    } else {
      btnIcon = (
        <IoMdEye size="25" style={{ color: "#4689CC", cursor: "pointer" }} />
      );
  ...
  return (
    <div className="add-to-today" onClick={onClick}>
      <div className="inside-wrap">
        ...
        <div className="add-to-today-btn">{btnIcon}</div>
      </div>
    </div>
  );
};
